...
// 16 nyu sites
const cities = [
  "Abu Dhabi", "Accra", "Berlin", "Buenos Aires", 
  "Florence", "London", "Los Angeles", "Madrid",
  "New York", "Paris", "Prague", "Shanghai",
  "Sydney", "Tel Aviv", "Tulsa", "Washington DC"
];
...
function updateWeatherData() {
  ...
  loadJSON(url, (response) => {
    ...
    if (json && json.main && json.main.temp) {
      ...
      if (isDay) {
        bgColor = (w.includes("rain") || w.includes("snow")) 
      ...
      pentatonic = getPentatonicFromRoot(rootNote, selectedScale)
        .map(f => f * shiftMultiplier);
    }
  });
}
  ...
  for (let hand of hands) {
    if (hand.confidence > 0.1) {
      ...
      [[0,1,2,3,4], [0,5,6,7,8], [0,9,10,11,12], 
       [0,13,14,15,16], [0,17,18,19,20]].forEach(finger => {
        beginShape();
